"""
AWS Lambda Handler - Car Damage Detection (TensorFlow)
=======================================================
Accepts POST /predict with JSON: {"image": "<base64>", "filename": "x.jpg"}
...
def predict_from_bytes(image_bytes: bytes) -> dict:
    ...
    return {
        ...
        'color':           DAMAGE_COLORS.get(pred_class, '#7b2ff7'),
        'icon':            DAMAGE_ICONS.get(pred_class, '🔍'),
    }
...
def handler(event, context):
    ...
    if http_method == 'GET' and path in ('/', ''):
        return {'statusCode': 200, 'headers': {'Content-Type': 'text/html; charset=utf-8'}, 'body': read_html()}
    ...
    if http_method == 'POST' and path == '/predict':
        try:
            ...
            if not b64_image:
                return err(400, 'Missing "image" field')
